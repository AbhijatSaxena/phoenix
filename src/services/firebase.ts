...
export async function exportDatabase(): Promise<Record<string, unknown>> {
  const [
    ...
    getDoc(doc(db, 'regent', 'config')),
    getDocs(collection(db, 'regentEmis')),
    getDoc(doc(db, 'zerodha', 'config')),
    getDocs(collection(db, 'zerodhaEntries')),
    getDocs(collection(db, 'links')),
    getDocs(collection(db, 'sessions')),
    getDoc(doc(db, 'meta', 'rates')),
    getDoc(doc(db, 'meta', 'paymentModes')),
  ])
  ...
  return {
    ...
    regent:         { config: regentConfig.exists() ? regentConfig.data() : null },
    regentEmis:     regentEmisSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    zerodha:        { config: zerodhaConfig.exists() ? zerodhaConfig.data() : null },
    zerodhaEntries: zerodhaEntriesSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    links:          linksSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    sessions:       sessionsSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    meta: {
      rates:        rates.exists() ? rates.data() : null,
      paymentModes: paymentModes.exists() ? paymentModes.data() : null,
    },
  }
}
